...
public class TrackerPlacer : MonoBehaviour
{
    ...
    public int numberOfTrackersToPlace = 8;
    ...
    private void PlaceRandomTrackers()
    {
        if (possibleLocations.Count < numberOfTrackersToPlace)
        {
        ...
        for (int i = 0; i < numberOfTrackersToPlace; i++)
        {
            PlaceTrackers(shuffledLocations[i]);
        }
    }

    private void PlaceTrackers(GameObject location)
    {
        ...
        Instantiate(trackerPrefab, adjustedPosition, adjustedRotation);
    }
